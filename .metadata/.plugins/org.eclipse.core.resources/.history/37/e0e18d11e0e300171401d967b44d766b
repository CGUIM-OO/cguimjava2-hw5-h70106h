	...
	private void calculate_chips() {
		...
		if (dealer.getTotalValue() > 21 && players[0].getTotalValue() <= 21) {
			...
			System.out.println(players[0].getName() + " card value is " + players[0].getTotalValue() + " ,Get " + p1Bet
					+ " Chips, the Chips now is:" + players[0].getCurrentChips());// 印出玩家1的點數總和,得到的下注籌碼,目前的籌碼
		}

		// 莊家沒爆,玩家沒爆->比較誰最接近21點
		if (dealer.getTotalValue() <= 21 && players[0].getTotalValue() <= 21) {
			if (dealer.getTotalValue() == players[0].getTotalValue()) {// 平手的情況
			...
			if (dealer.getTotalValue() > players[0].getTotalValue()) {// 莊家較接近21點->莊家贏
				players[0].increaseChips(-p1Bet);// 沒收玩家1的下注籌碼
				System.out.println(players[0].getName() + " card value is " + players[0].getTotalValue() + ", Loss "
						+ p1Bet + " Chips, the Chips now is:" + players[0].getCurrentChips());// 印出玩家1的點數總和,輸的籌碼,現在剩的籌碼
			}
			if (dealer.getTotalValue() < players[0].getTotalValue()) {// 玩家較接近21點->玩家贏
				players[0].increaseChips(p1Bet);
				System.out.println(players[0].getName() + " card value is " + players[0].getTotalValue() + " ,Get "
						+ p1Bet + " Chips, the Chips now is:" + players[0].getCurrentChips());// 印出玩家1的點數總和,得到的下注籌碼,目前的籌碼
			}
		...
		// 莊家沒爆,玩家爆->莊家贏
		if (dealer.getTotalValue() <= 21 && players[0].getTotalValue() > 21) {
			players[1].increaseChips(-p2Bet);// 沒收玩家2的下注籌碼
			...
			// 印出玩家2的點數總和,輸的籌碼,現在剩的籌碼
		}

		// 莊家爆,玩家沒爆->玩家贏
		if (dealer.getTotalValue() > 21 && players[1].getTotalValue() <= 21) {
			players[1].increaseChips(p2Bet);// 賠玩家與下注籌碼相符之籌碼
			System.out.println(players[1].getName() + " card value is " + players[1].getTotalValue() + " ,Get " + p2Bet
					+ " Chips, the Chips now is:" + players[1].getCurrentChips());// 印出玩家2的點數總和,得到的下注籌碼,目前的籌碼
		}

		// 莊家沒爆,玩家沒爆->比較誰最接近21點
		if (dealer.getTotalValue() <= 21 && players[1].getTotalValue() <= 21) {
			if (dealer.getTotalValue() == players[1].getTotalValue()) {// 平手的情況
			...
			if (dealer.getTotalValue() > players[1].getTotalValue()) {// 莊家較接近21點->莊家贏
				players[1].increaseChips(-p2Bet);// 沒收玩家2的下注籌碼
				System.out.println(players[1].getName() + " card value is " + players[1].getTotalValue() + ", Loss "
						+ p2Bet + " Chips, the Chips now is:" + players[1].getCurrentChips());// 印出玩2的點數總和,輸的籌碼,現在剩的籌碼
			}
			if (dealer.getTotalValue() < players[1].getTotalValue()) {// 玩家較接近21點->玩家贏
				players[1].increaseChips(p2Bet);
				System.out.println(players[1].getName() + " card value is " + players[1].getTotalValue() + " ,Get "
						+ p2Bet + " Chips, the Chips now is:" + players[1].getCurrentChips());// 印出玩家2的點數總和,得到的下注籌碼,目前的籌碼
			}
		...
		if (dealer.getTotalValue() <= 21 && players[2].getTotalValue() > 21) {
			players[2].increaseChips(-p3Bet);// 沒收玩家3的下注籌碼
			...
			// 印出玩家3的點數總和,輸的籌碼,現在剩的籌碼
		}

		// 莊家爆,玩家沒爆->玩家贏
		if (dealer.getTotalValue() > 21 && players[2].getTotalValue() <= 21) {
			players[2].increaseChips(p3Bet);// 賠玩家與下注籌碼相符之籌碼
			System.out.println(players[2].getName() + " card value is " + players[2].getTotalValue() + " ,Get " + p3Bet
					+ " Chips, the Chips now is:" + players[2].getCurrentChips());// 印出玩家1的點數總和,得到的下注籌碼,目前的籌碼
		}

		// 莊家沒爆,玩家沒爆->比較誰最接近21點
		if (dealer.getTotalValue() <= 21 && players[2].getTotalValue() <= 21) {
			if (dealer.getTotalValue() == players[2].getTotalValue()) {// 平手的情況
			...
			if (dealer.getTotalValue() > players[2].getTotalValue()) {// 莊家較接近21點->莊家贏
				players[2].increaseChips(-p3Bet);// 沒收玩家1的下注籌碼
				System.out.println(players[2].getName() + " card value is " + players[2].getTotalValue() + ", Loss "
						+ p3Bet + " Chips, the Chips now is:" + players[2].getCurrentChips());// 印出玩家3的點數總和,輸的籌碼,現在剩的籌碼
			}
			if (dealer.getTotalValue() < players[2].getTotalValue()) {// 玩家較接近21點->玩家贏
				players[2].increaseChips(p2Bet);
				System.out.println(players[2].getName() + " card value is " + players[2].getTotalValue() + " ,Get "
						+ p3Bet + " Chips, the Chips now is:" + players[2].getCurrentChips());// 印出玩家3的點數總和,得到的下注籌碼,目前的籌碼
			}
		...
		if (dealer.getTotalValue() <= 21 && players[3].getTotalValue() > 21) {
			players[3].increaseChips(-p4Bet);// 沒收玩家3的下注籌碼
		...
		// 莊家爆,玩家沒爆->玩家贏
		if (dealer.getTotalValue() > 21 && players[3].getTotalValue() <= 21) {
			players[3].increaseChips(p4Bet);// 賠玩家與下注籌碼相符之籌碼
			System.out.println(players[3].getName() + " card value is " + players[3].getTotalValue() + " ,Get " + p4Bet
					+ " Chips, the Chips now is:" + players[3].getCurrentChips());// 印出玩家4的點數總和,得到的下注籌碼,目前的籌碼
		}

		// 莊家沒爆,玩家沒爆->比較誰最接近21點
		if (dealer.getTotalValue() <= 21 && players[3].getTotalValue() <= 21) {
			if (dealer.getTotalValue() == players[3].getTotalValue()) {// 平手的情況
			...
			if (dealer.getTotalValue() > players[3].getTotalValue()) {// 莊家較接近21點->莊家贏
				players[3].increaseChips(-p4Bet);// 沒收玩家1的下注籌碼
				System.out.println(players[3].getName() + " card value is " + players[3].getTotalValue() + ", Loss "
						+ p4Bet + " Chips, the Chips now is:" + players[3].getCurrentChips());// 印出玩家4的點數總和,輸的籌碼,現在剩的籌碼
			}
			if (dealer.getTotalValue() < players[3].getTotalValue()) {// 玩家較接近21點->玩家贏
				players[3].increaseChips(p4Bet);
				System.out.println(players[3].getName() + " card value is " + players[3].getTotalValue() + " ,Get "
						+ p4Bet + " Chips, the Chips now is:" + players[3].getCurrentChips());// 印出玩家4的點數總和,得到的下注籌碼,目前的籌碼
			}
